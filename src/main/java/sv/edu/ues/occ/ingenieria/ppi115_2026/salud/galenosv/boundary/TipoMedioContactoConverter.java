package sv.edu.ues.occ.ingenieria.ppi115_2026.salud.galenosv.boundary;

import jakarta.faces.component.UIComponent;
import jakarta.faces.context.FacesContext;
import jakarta.faces.convert.Converter;
import jakarta.faces.convert.FacesConverter;
import java.util.UUID;
import sv.edu.ues.occ.ingenieria.ppi115_2026.salud.galenosv.entity.TipoMedioContacto;

@FacesConverter("tipoMedioContactoConverter")
public class TipoMedioContactoConverter implements Converter<TipoMedioContacto> {

    @Override
    public TipoMedioContacto getAsObject(
            FacesContext context,
            UIComponent component,
            String value) {

        if (value == null || value.isBlank()) {
            return null;
        }

        return new TipoMedioContacto(UUID.fromString(value));
    }

    @Override
    public String getAsString(
            FacesContext context,
            UIComponent component,
            TipoMedioContacto value) {

        if (value == null || value.getIdTipoMedioContacto() == null) {
            return "";
        }

        return value.getIdTipoMedioContacto().toString();
    }
}
